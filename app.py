...
class LibraryGUI:
    def __init__(self, master):
        self.master = master
        self.master.title("Gerenciador Biblioteca JK")
...
class MainScreens(Frame):
    def __init__(self, gui):
        ...
        Label(self, text="Bem-vindo à Biblioteca JK", **label_style).pack(pady=40)
        Label(self, text="Menu principal", **label_style).pack(pady=40)
        Button(self, text="Área do locatário", command=lambda: gui.switch_frame(TenantFunctions), **main_button_style).pack(pady=20)
        Button(self, text="Área do administrador", command=lambda: admin_login(gui), **main_button_style).pack(pady=20)
...
class AdminPanel(Frame):
    def __init__(self, gui):
        ...
        Button(self, text="Ver Aluguéis em Atraso", command=lambda: view_overdue_books(gui), **button_style).pack(pady=10)
...
def check_and_rent_book(gui):
    def show_dialog():
        ...
        dialog.grab_set()
        dialog.focus_set()
        dialog.lift()  # Bring to the front

        content_frame = Frame(dialog, bg=background_color)
