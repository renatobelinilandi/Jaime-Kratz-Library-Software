    ...
    def add_book(self, title, author, quantity, genre):
        for book in self.books:
            if self._normalize_text(book['title']) == self._normalize_text(title) and \
                ...
                self.save_data()
                write_to_log(
                    f"Updated quantity for existing book: {title} by {author}, new quantity: {book['quantity']}")
    ...
    def find_books_by_title_author_and_genre(self, search_text, genre):
        ...
        if genre == "qualquer":
            return [book for book in self.books if
                    search_text in self._normalize_text(book['title']) or search_text in self._normalize_text(
                        book['author'])]
        else:
            return [book for book in self.books if (
                        search_text in self._normalize_text(book['title']) or search_text in self._normalize_text(
                    book['author'])) and genre in self._normalize_text(book['genre'])]
