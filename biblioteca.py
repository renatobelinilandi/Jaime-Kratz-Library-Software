...
class Library:
    ...
    def renew_loan(self, tenant_name):
        tenant_name = tenant_name.upper()  # Normalizando o nome para maiúsculas
        for book in self.books:
            for loan in book['loans']:
                if loan['tenant_name'] == tenant_name and not loan['returned']:
...
class TenantFunctions(Frame):
    def __init__(self, gui):
        ...
        Button(self, text="Alugar Livro", command=lambda: show_book_selection(gui), height=3, width=25, **button_style).pack(pady=10)
...
def renew_loan_gui(gui):
    ...
    if tenant_name:
        ...
        if book_title:
            ...
            if response:
                messagebox.showinfo("Renovação de Aluguel", f"Aluguel do livro '{book_title}' renovado com sucesso.")
        else:
