...
class AdminPanel(Frame):
    def __init__(self, gui):
        ...
        Button(self, text="Remover Livro", command=lambda: remove_book(gui), height=3, width=25,
               **button_style).pack(pady=10)
        Button(self, text="Buscar Aluguéis Ativos", command=lambda: search_active_loans(gui), height=3, width=25,
               **button_style).pack(pady=10)
        Button(self, text="Ver Livros em Atraso", command=lambda: view_overdue_books(gui), height=3, width=25,
               **button_style).pack(pady=10)
        Button(self, text="Trocar Senha de Admin", command=lambda: change_admin_password(gui), height=3, width=25,
               **button_style).pack(pady=10)
...
class TenantFunctions(Frame):
    def __init__(self, gui):
        ...
        Button(self, text="Buscar Livros", command=lambda: search_books(gui), height=3, width=25, **button_style).pack(pady=10)
        Button(self, text="Alugar Livro", command=lambda: check_and_rent_book(gui), height=3, width=25, **button_style).pack(pady=10)
        Button(self, text="Renovar Aluguel", command=lambda: renew_loan_gui(gui), height=3, width=25, **button_style).pack(pady=10)
        Button(self, text="Devolver Livro", command=lambda: return_book(gui), height=3, width=25, **button_style).pack(pady=10)
        Button(self, text="Voltar ao Principal", command=lambda: gui.switch_frame(MainScreens), height=3, width=25, **button_style).pack(pady=20)
...
def check_and_rent_book(gui):
    ...
    if tenant_name:
        tenant_name = tenant_name.upper()  # Normalize the name to uppercase
        # First check if the tenant already has an active loan
        if any(loan for book in gui.library.books for loan in book['loans'] if
               loan['tenant_name'] == tenant_name and not loan['returned']):
            messagebox.showerror("Erro de Aluguel",
                                 "Você já tem um livro alugado. Devolva o livro atual antes de alugar um novo.")
            return

        room_number = simpledialog.askstring("Alugar Livro", "Digite o número da sua sala ou função:")
        if not room_number:
            messagebox.showinfo("Aluguel de Livro", "Você não digitou o número da sala ou função.")
            return
        ...
        if title:
            ...
            if books_found:
                ...
                show_book_selection(gui, books_found, tenant_name, room_number)
            else:
...
def show_book_selection(gui, books_found, tenant_name, room_number):
    ...
    def select_book():
        ...
        if selected_index:
            ...
            result = gui.library.rent_book(selected_book['title'], selected_book['author'], tenant_name, room_number.upper())
